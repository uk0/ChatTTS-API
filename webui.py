...
def tts_api(text, temperature=0.3, top_P=0.7, top_K=20, audio_seed_input=2, text_seed_input=42, refine_text_flag=True):
    (sample_rate, audio_data), text_data = generate_audio(text, temperature, top_P, top_K, audio_seed_input,
                                                          text_seed_input, refine_text_flag)
...
def main():
    ...
    def api_tts(request):
        ...
        text_seed_input = data.get("text_seed_input", 42)
        refine_text_flag = data.get("refine_text_flag", True)

        sample_rate, audio_data = tts_api(text, temperature, top_P, top_K, audio_seed_input, text_seed_input,
                                          refine_text_flag)
        return gr.routes.App.serialize_audio_data(sample_rate, audio_data)
